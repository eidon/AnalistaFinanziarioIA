...
let idTransazioneInModifica = null;
export async function salvaTransazioneModale() {
    const titoloIdEl = document.getElementById('titolo-selezionato-id');
    const titoloJsonEl = document.getElementById('titolo-selezionato-json');
    ...
    const transazioneDto = {
        utenteId: window.UTENTE_ID,
        titoloId: titoloId,
        titoloLookup: titoloLookup,
        tipoOperazione: document.getElementById('m-tipo-operazione').value,
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (window.chiudiModale)
                window.chiudiModale();
            await caricaPortafoglio();
        }
        else {
            if (typeof window.gestisciErroreServer === 'function') {
                await window.gestisciErroreServer(response);
            }
            else {
...
export async function eliminaTrasazione(id) {
    // Chiediamo conferma prima di procedere
    if (!confirm("Sei sicuro di voler eliminare questa transazione? Questa azione ricalcolerà il tuo portafoglio."))
        return;
    try {
...
export function apriModaleNuova() {
    ...
    window.idTransazioneInModifica = null;
    ...
    if (modalTitle)
        modalTitle.innerText = "Nuova Transazione";
    const btnSalva = document.getElementById('btn-salva-transazione');
    if (btnSalva)
        btnSalva.innerText = "Salva Transazione";
    ...
    if (typeof window.setTipoOperazione === 'function') {
        window.setTipoOperazione('Acquisto');
    }
    // 6. Apri il modale (Casting ad any per correggere l'errore TS)
    window.apriModale();
}
export async function preparaModificaTransazione(id) {
    try {
        const res = await fetch(`/api/transazioni/${id}`);
        if (!res.ok)
        ...
        // 3. Popoliamo i campi
        document.getElementById('ricerca-titolo').value = t.assetPortafoglio?.titolo?.nome || "";
        document.getElementById('m-quantita').value = t.quantita;
        document.getElementById('m-prezzo').value = t.prezzoUnitario;
        document.getElementById('m-commissioni').value = t.commissioni || 0;
        document.getElementById('m-tasse').value = t.tasse || 0;
        const campoNote = document.getElementById('m-note');
        if (campoNote)
            campoNote.value = t.note || "";
        const campoIsin = document.getElementById('m-isin');
        if (campoIsin)
            campoIsin.value = t.assetPortafoglio?.titolo?.isin || "";
        if (t.data) {
            document.getElementById('m-data').value = t.data.split('T')[0];
        }
        ...
        const btnSalva = document.getElementById('btn-salva-transazione');
        if (btnSalva)
            btnSalva.innerText = "Aggiorna Transazione";
        // 5. Apriamo il modale
        window.apriModale();
    }
    catch (error) {
...
export async function caricaPortafoglio() {
    const UTENTE_ID = window.UTENTE_ID || "C206BEC1-A5DE-4896-8472-3FF512635B3A";
    ...
    try {
        ...
        if (!resDashboard.ok)
            throw new Error(`Errore API Dashboard: ${resDashboard.status}`);
        ...
        if (tbodyStoria) {
            tbodyStoria.innerHTML = storia.map((t) => {
        ...
        if (window.aggiornaGrafico && assets.length > 0) {
            const labels = assets.map((a) => a.ticker);
            const values = assets.map((a) => a.valoreDiMercato); // Meglio usare valore di mercato per il grafico a torta
            window.aggiornaGrafico(labels, values);
        }
 ...
 */
function aggiornaSpecchiettoPerformance(stats) {
    ...
    if (elCapitale)
        elCapitale.textContent = format(stats.capitaleInvestito || 0);
    const elCosti = document.getElementById('perf-costi');
    if (elCosti)
        elCosti.textContent = "-" + format(stats.costiTotali || 0);
...
let timerDebounce;
export async function eseguiRicerca(testo) {
    const lista = document.getElementById('lista-suggerimenti');
    if (!lista)
        return;
    // Capiamo se siamo in modalità Acquisto o Vendita
    const tipoOperazione = document.getElementById('m-tipo-operazione').value;
    // --- CASO VENDITA: Cerchiamo solo nei nostri titoli (veloce, locale) ---
    if (tipoOperazione === 'Vendita') {
        const filtrati = cacheMieiAssets.filter(a => a.simbolo.toLowerCase().includes(testo.toLowerCase()) ||
...
function renderizzaSuggerimenti(items, isVendita) {
    const lista = document.getElementById('lista-suggerimenti');
    if (!lista)
        return;
...
export async function selezionaTitolo(dataB64, quantitaPortafoglio = 0) {
    ...
    const inputId = document.getElementById('titolo-selezionato-id');
    ...
    const inputJson = document.getElementById('titolo-selezionato-json');
    if (inputJson) {
    ...
    const selectTipo = document.getElementById('tipo-operazione');
    const inputQuantita = document.getElementById('m-quantita'); // Verifica l'ID esatto del tuo input quantità
    ...
    if (!idDaUsare) {
        try {
            ...
            if (res.ok) {
                ...
                if (inputJson)
                    inputJson.value = JSON.stringify(completo);
                const inputIsin = document.getElementById('m-isin');
                if (inputIsin)
                    inputIsin.value = completo.isin || "";
            }
        }
        catch (e) {
            console.error(e);
        ...
        const inputIsin = document.getElementById('m-isin');
        if (inputIsin)
            inputIsin.value = titoloSelezionato.isin || "";
    }
}
// --- 4. CHAT AI ---
...
export function appendMessage(testo, mittente) {
    ...
    div.innerHTML = `
        ...
        : 'bg-gray-700 text-gray-100 rounded-tl-none border border-gray-600'} shadow-md text-sm">
            ${testoFormattato}
        </div>
    `;
...
export function mostraBenvenutoStatico() {
    ...
    setTimeout(() => {
        ...
        window.appendMessage(messaggio, 'ai');
    }, 800);
}
...
async function setTipoOperazione(tipo) {
    const inputTipo = document.getElementById('m-tipo-operazione');
    ...
    if (tipo === 'Acquisto') {
        ...
        try {
            const uid = window.UTENTE_ID || "C206BEC1-A5DE-4896-8472-3FF512635B3A";
...
window["caricaPortafoglio"] = caricaPortafoglio;
window["salvaTransazioneModale"] = salvaTransazioneModale;
window["apriModale"] = () => {
...
window["selezionaTitolo"] = selezionaTitolo;
window["chiediAI"] = chiediAI;
window["appendMessage"] = appendMessage;
window["mostraBenvenutoStatico"] = mostraBenvenutoStatico;
window["eliminaTrasazione"] = eliminaTrasazione;
window["preparaModificaTransazione"] = preparaModificaTransazione;
window["setTipoOperazione"] = setTipoOperazione;
window["apriModaleNuova"] = apriModaleNuova;
document.getElementById('chat-input')?.addEventListener('keypress', (e) => {
    if (e.key === 'Enter')
        chiediAI();
});
document.getElementById('btn-chiedi-ai')?.addEventListener('click', chiediAI);
//# sourceMappingURL=Portafoglio.js.map